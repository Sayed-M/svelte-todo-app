<script lang="ts">
    ...
    let createFolder = () => {
        if (folderName) {
            ...
            folders.createFolder(newFolder);
    ...
    <form on:submit|preventDefault={() => createFolder()}>
        <div class="form-control">
            <div class="input-group">
                <input type="text" placeholder="Folder name..." bind:value={folderName} class="input w-full max-w-xs p-1 text-sm focus:outline-none" />
